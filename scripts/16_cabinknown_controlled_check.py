...
REPORT_PATH = REPORTS_DIR / "16_cabinknown_controlled_check.md"
CSV_PATH = REPORTS_DIR / "16_cabinknown_controlled_check.csv"
DIFF_ROWS_PATH = REPORTS_DIR / "16_cabinknown_diff_rows.csv"
...
VARIANTS = [
    VariantSpec(
        ...
        output_file="",
        purpose="current clean GB baseline reference",
        submission_candidate=False,
    ),
    VariantSpec(
        variant="raw_plus_cabinknown",
        candidate_id="raw_plus_cabinknown__GradientBoostingClassifier",
        feature_set="raw_plus_cabinknown",
        features=[*RAW_TABULAR, "CabinKnown"],
        output_file="submission_16a_gb_raw_plus_cabinknown.csv",
        purpose="controlled check of simple binary CabinKnown on top of raw_tabular",
...
def _status_for_candidate(
    ...
    test_changed_pct: object,
    calibration_flag: str,
) -> str:
    train_better = cv_delta > 0 and oof_delta > 0
    not_worse = cv_delta >= -MEANINGFUL_WORSE_TOLERANCE and oof_delta >= -MEANINGFUL_WORSE_TOLERANCE
    pred_not_inflated = pred_1_rate_delta <= PRED_RATE_INFLATION_MARGIN
    suspicious_test_diff = isinstance(test_changed_pct, float) and test_changed_pct > SUSPICIOUS_TEST_DIFF_PCT

    if net > 0 and (train_better or not_worse) and pred_not_inflated:
        if calibration_flag == "RISK_TEST_PRED_RATE_ABOVE_TRAIN_SURVIVAL":
            return "HOLD_FOR_MANUAL_REVIEW"
        if suspicious_test_diff and not train_better:
            return "HOLD_FOR_MANUAL_REVIEW"
...
def _sanity_rows(test: pd.DataFrame, rows: list[dict[str, object]]) -> list[dict[str, object]]:
    sanity = []
    output_path = SUBMISSIONS_DIR / "submission_16a_gb_raw_plus_cabinknown.csv"
    row_by_variant = {str(row["variant"]): row for row in rows}
    candidate_row = row_by_variant["raw_plus_cabinknown"]
    expected_generated = candidate_row["status"] == "KEEP_CANDIDATE"
    exists = output_path.exists()
    sanity.append(
        {
            "check": "submission_16a existence follows train-side status",
            "status": "PASS" if exists == expected_generated else "FAIL",
            "detail": f"status={candidate_row['status']}; exists={exists}; file={_relative(output_path)}",
        }
    )
    if exists:
...
def _build_report(
    ...
    lines = [
        ...
        f"- Suspicious test-diff threshold without strong train support: `{SUSPICIOUS_TEST_DIFF_PCT}%`.",
        "",
        "## Model panel",
        "",
...
def main() -> None:
    ...
    for variant in VARIANTS:
        prediction, _ = _fit_full_predict(variant, train, test)
